...
async function scraper(key) {
  ...
  Promise.map(data.products, async(product) => {
    ...
    if (downloadData.thumbnail) {
      ...
      model.post_file['path'] = `https://kemono.party/gumroad/files/${userId}/${product.id}/${filename}`
    }
        ...
        request2
          ...
          .on('complete', async(res) => {
            ...
            model.attachments.push({
              name: `${filename}.${ext}`,
              path: `https://kemono.party/gumroad/attachments/${userId}/${product.id}/${filename}.${ext}`
            });
            await fs.move(
